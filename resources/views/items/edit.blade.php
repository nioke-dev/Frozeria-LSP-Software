...
<div class="max-w-4xl mx-auto pb-20 px-4" 
    x-data="{ 
        ...
        item_code: '{{ old('item_code', $item->item_code) }}',
        codeError: '',
        isCheckingCode: false,

        // Category selection
        category_id: '{{ old('category_id', $item->category_id) }}',
        categoryName: '{{ $categories->find(old('category_id', $item->category_id))?->name ?? 'Pilih kategori' }}',
        ...
        async checkCodeAvailability() {
            ...
            try {
                const response = await fetch(`{{ route('items.check-code') }}?code=${this.item_code}&exclude_id={{ $item->id }}`);
    ...
    <template x-teleport="body">
        <div x-show="showError" 
            ...
            <div class="bg-white border-2 border-red-100 shadow-2xl shadow-red-200/50 rounded-[24px] p-5 flex items-center gap-4 pointer-events-auto">
                ...
                <div class="flex-grow">
                    <h4 class="text-sm font-black text-red-900 uppercase tracking-widest">Opps! Terjadi Kesalahan</h4>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-black text-gray-900 tracking-tight">Edit Barang</h1>
    ...
    <div class="bg-white rounded-[32px] border border-gray-100 shadow-xl shadow-gray-200/40 overflow-hidden">
        <form action="{{ route('items.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="p-6 md:p-8 space-y-8"
            @submit="
            ...
            <div class="space-y-8">
                        ...
                        <label for="image" class="flex-grow h-24 cursor-pointer group"
                            ...
                            <div class="w-full h-full border-2 border-dashed rounded-2xl flex flex-col items-center justify-center gap-1 transition-all duration-300"
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-black text-blue-800 tracking-tight" x-text="isDragging ? 'Lepaskan foto...' : 'Klik atau Drag foto baru'"></span>
                                </div>
                                <span class="text-[9px] font-bold text-blue-400 uppercase tracking-widest">JPG, PNG — Maks. 2 MB</span>
                            </div>
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <input type="text" name="item_code" x-model="item_code" @input.debounce.500ms="checkCodeAvailability()" required
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" name="min_stock" value="{{ old('min_stock', $item->min_stock) }}" required 
                                class="w-full px-5 py-3 bg-gray-50/50 border-2 border-gray-50 rounded-xl focus:bg-white focus:border-blue-500 outline-none transition-all text-gray-900 font-bold text-sm">
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="description" rows="2" 
                            class="w-full px-5 py-3 bg-gray-50/50 border-2 border-gray-50 rounded-xl focus:bg-white focus:border-blue-500 outline-none transition-all text-gray-900 font-bold text-sm">{{ old('description', $item->description) }}</textarea>
                    </div>
                </div>
                ...
                <div class="pt-6 border-t border-gray-50 flex items-center justify-between">
                    <div class="hidden sm:flex items-center gap-2 text-gray-400">
                        ...
                        <p class="text-[10px] font-bold italic">Lengkapi semua data yang diperlukan.</p>
                    </div>
