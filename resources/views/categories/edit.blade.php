...
<div class="max-w-4xl mx-auto pb-20 px-4">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            ...
            <p class="text-gray-500 text-sm font-bold mt-1">Perbarui informasi kategori <span class="text-orange-600 font-black italic">{{ $category->name }}</span></p>
        </div>
    ...
    <div class="bg-white rounded-[40px] border border-gray-100 shadow-2xl shadow-gray-200/40 overflow-hidden">
        <form action="{{ route('categories.update', $category->id) }}" method="POST" class="p-8 md:p-12 space-y-10">
            @csrf
            @method('PUT')
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative group">
                        <input type="text" name="name" value="{{ old('name', $category->name) }}" required
                            class="w-full h-16 px-6 bg-gray-50/50 border-2 border-transparent focus:border-orange-500 focus:bg-white rounded-2xl outline-none transition-all text-gray-900 font-black text-lg placeholder:text-gray-200 shadow-sm"
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative group">
                        <textarea name="description" rows="5" 
                            class="w-full px-6 py-5 bg-gray-50/50 border-2 border-transparent focus:border-orange-500 focus:bg-white rounded-3xl outline-none transition-all text-gray-700 font-bold text-sm placeholder:text-gray-200 shadow-sm"
                            placeholder="Jelaskan secara singkat kegunaan kategori ini...">{{ old('description', $category->description) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="pt-10 border-t border-gray-50 flex flex-col sm:flex-row items-center justify-between gap-6">
                <div class="flex items-center gap-2 text-gray-400">
                    ...
                    <p class="text-[10px] font-black italic tracking-wider">Pastikan nama kategori unik dan belum terdaftar.</p>
                </div>
